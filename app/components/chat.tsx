...
export function Avatar(props: { role: Message["role"] }) {
  ...
  function customGetEmojiUrl(unified: string) {
    return `https://gcore.jsdelivr.net/npm/emoji-datasource-apple/img/apple/64/` + unified + ".png";
  }

  return (
    <div className={styles["user-avtar"]}>
      <Emoji unified={config.avatar} size={18} getEmojiUrl={customGetEmojiUrl} />
    </div>
  );
}
...
export function Chat(props: {
  ...
  // preview messages
  const nonSys = session.messages.filter((msg) => !msg.isError && msg.role != 'system');
  const messages = context
    .concat(nonSys as RenderMessage[])
    .concat(
      isLoading
        ? [
          {
      ...
      <div className={styles["window-header"]}>
        ...
        >
          ...
          <div className={styles["window-header-sub-title"]}>
            {Locale.Chat.SubTitle(session.messages.length - 1)}
          </div>
        </div>
        <div className={styles["window-actions"]}>
          ...
          <div className={styles["window-action-button"]}>
            <IconButton
              ...
              onClick={() => {
                exportMessages(
                  session.messages.filter((msg) => !msg.isError && msg.role != 'system'),
                  session.topic,
                );
              }}
            />
